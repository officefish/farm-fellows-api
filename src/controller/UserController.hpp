#ifndef FARM_FELLOWS_USER_CONTROLLER_HPP
#define FARM_FELLOWS_USER_CONTROLLER_HPP
...
public:
  ...
  ENDPOINT_INFO(getUserMinimumById) {
    info->summary = "Get one User minimum info by userId";

    info->addResponse<Object<UserMinimumDto>>(Status::CODE_200, "application/json");
    info->addResponse<Object<StatusDto>>(Status::CODE_404, "application/json");
    info->addResponse<Object<StatusDto>>(Status::CODE_500, "application/json");

    info->pathParams["userId"].description = "User Identifier";
  }
   ENDPOINT("GET", "users/{userId}/minimum", getUserMinimumById,
           PATH(Int32, userId))
  {
    return createDtoResponse(Status::CODE_200, m_userService.getUserMinimumById(userId));
  }
  ...
  ENDPOINT_INFO(getUsersMinimum) {
    info->summary = "get all stored users with minimum info";

    info->addResponse<oatpp::Object<UsersPageDto>>(Status::CODE_200, "application/json");
    info->addResponse<Object<StatusDto>>(Status::CODE_500, "application/json");
  }
  ENDPOINT("GET", "users/offset/{offset}/limit/{limit}/minimum", getUsersMinimum,
           PATH(UInt32, offset),
           PATH(UInt32, limit))
  {
    return createDtoResponse(Status::CODE_200, m_userService.getAllUsersMinimum(offset, limit));
  }
...
*/
#endif /* FARM_FELLOWS_USER_CONTROLLER_HPP */
