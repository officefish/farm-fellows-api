...
oatpp::Object<UserDto> UserService::getUserById(const oatpp::Int32& id, const oatpp::provider::ResourceHandle<oatpp::orm::Connection>& connection) {
  ...
  auto minimum = result[0];  
  return minimum;

}

oatpp::Object<UserMinimumDto> UserService::getUserMinimumById(const oatpp::Int32& id, const oatpp::provider::ResourceHandle<oatpp::orm::Connection>& connection) {

  auto dbResult = m_database->getUserMinimumById(id, connection);
  OATPP_ASSERT_HTTP(dbResult->isSuccess(), Status::CODE_500, dbResult->getErrorMessage());
  OATPP_ASSERT_HTTP(dbResult->hasMoreToFetch(), Status::CODE_404, "User not found");
  ...
  auto minimum = static_cast<oatpp::Object<UserMinimumDto>>(result[0]);  
  return minimum;
...
oatpp::Object<PageDto<oatpp::Object<UserMinimumDto>>> UserService::getAllUsersMinimum(const oatpp::UInt32& offset, const oatpp::UInt32& limit) {
  ...
  auto dbResult = m_database->getAllUsersMinimum(offset, countToFetch);
  OATPP_ASSERT_HTTP(dbResult->isSuccess(), Status::CODE_500, dbResult->getErrorMessage());

  auto items = dbResult->fetch<oatpp::Vector<oatpp::Object<UserMinimumDto>>>();

  auto page = PageDto<oatpp::Object<UserMinimumDto>>::createShared();
  page->offset = offset;
  page->limit = countToFetch;
  page->count = items->size();
  page->items = items;
